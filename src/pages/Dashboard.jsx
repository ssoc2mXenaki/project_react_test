...
const Dashboard = () => {
  ...
  console.log("ta 5 courses einai:", course.slice(0,5));
    ...
    <div style={{marginLeft: '30px',marginTop: '20px', marginRight: '30px', border:'solid lightgrey 1px', borderRadius:6}}>
        ...
        <div style={{backgroundColor:'#f8fafa', borderBottom: 'solid lightgrey 1px', borderTopLeftRadius:6, borderTopRightRadius:6}}><div style={{marginLeft:'10px', paddingTop: '10px',paddingBottom:'10px'}}>Last 5 courses</div></div>
          <Table striped>
            ...
            {course.slice(0,5).map(courses => {
             return (<tr key={courses.id}>
                ...
                <td>{courses.title}</td>
                <td>{courses.online ? <FontAwesomeIcon icon="fa-solid fa-check" color='green' /> : <FontAwesomeIcon icon="fa-solid fa-xmark" />}</td>
                <td>{courses.price.normal}</td>
                <td>{moment(courses.dates.start_date).format('DD/MM/YYYY')} - {moment(courses.dates.end_date).format('DD/MM/YYYY')}</td>
                <td><Button style={{ backgroundColor:"#0dcaf0", color:"black",border:'none', float:'right',}} href={"/CoursesDetails?id="+courses.id}>View details</Button></td>
              </tr>)
              //}  
            })}
             </tbody>
          ...
          <hr />
          <div style={{backgroundColor:'#f8f9fa', overflow:'hidden', marginBottom:'15px'}}><Button style={{float:'right', marginRight:'10px'}} href="/Courses"> View all</Button></div>
    </div>
    </>
      );
  }
